...
mod annotation_service_tests {
  ...
  fn test_get_all_annotation_nodes_basic() {
    ...
    if let Some(root) = ts_file.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
    ...
    if let Some(root) = ts_file.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
      ...
      for annotation in annotations {
        if let Some(name_node) = get_annotation_name_node(&ts_file, annotation) {
          if let Some(name_text) = ts_file.get_text_from_node(&name_node) {
            assert!(!name_text.is_empty(), "Annotation name should not be empty");
            assert!(name_text.chars().all(|c| c.is_alphanumeric() || c == '_'), "Name should be valid identifier");
          }
        }
      }
    }
  ...
  fn test_add_annotation_argument_basic() {
    ...
    let result = add_annotation_argument(
      &mut ts_file,
      annotation_pos,
      "name",
      "\"users\""
  ...
  fn test_add_annotation_single_value_basic() {
    ...
    let result = add_annotation_single_value(
      &mut ts_file,
      annotation_pos,
      "\"test value\""
    );
...
"#;
  ...
  fn test_get_all_annotation_nodes_complex() {
    ...
    if let Some(root) = ts_file.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
      ...
      assert!(annotations.len() >= 10, "Should find multiple annotations, found: {}", annotations.len());
      
      let annotation_texts: Vec<String> = annotations.iter()
  ...
  fn test_get_annotation_argument_key_nodes_complex() {
    ...
    if let Some(root) = ts_file.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
      ...
      assert!(keys.len() > 0, "Should find annotation argument keys");
      
      let key_texts: Vec<String> = keys.iter()
        .filter_map(|node| ts_file.get_text_from_node(node))
        .map(|text| text.to_string())
        .collect();
      ...
      assert!(key_texts.iter().any(|text| text == "nullable" || text == "strategy" || text == "min"), 
  ...
  fn test_get_annotation_argument_value_nodes_complex() {
    ...
    if let Some(root) = ts_file.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
      ...
      assert!(values.len() > 0, "Should find annotation argument values");
      
      let value_texts: Vec<String> = values.iter()
        .filter_map(|node| ts_file.get_text_from_node(node))
        .map(|text| text.to_string())
        .collect();
      
      // Should find string values, boolean values, enum values, etc.
      assert!(value_texts.iter().any(|text| text.contains("\"users\"") || text.contains("\"username\"")), 
              "Should find string values");
      assert!(value_texts.iter().any(|text| text == "false" || text == "true" || text.contains("GenerationType")), 
  ...
  fn test_find_annotation_value_node_by_key_complex() {
    ...
    if let Some(root) = ts_file.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
      ...
      if let Some(value_node) = name_value {
        if let Some(value_text) = ts_file.get_text_from_node(&value_node) {
  ...
  fn test_annotation_functions_with_no_tree() {
    ...
    if let Some(root) = ts_file_with_tree.tree.as_ref().and_then(|tree| Some(tree.root_node())) {
  ...
  fn test_add_annotation_argument_to_existing_arguments() {
    ...
    assert!(ts_file.source_code.contains("name = \"username\""), "Should preserve existing argument");
  }
}