...
public class Test {}
      ...
      for import_node in &imports {
        assert_eq!(import_node.kind(), "import_declaration", "All nodes should be import_declaration type");
      }
    }
...
public class Test {}
  ...
  mod get_import_declaration_relative_import_scope_node_tests {
    ...
    fn test_deep_nested_import_scope() {
      ...
      let scope_text = ts_file.get_text_from_node(&result.unwrap()).expect("Should get scope text");
      assert_eq!(scope_text, "com.company.project.module.submodule.feature", "Should return full nested scope");
    }
  ...
  mod get_import_declaration_full_import_scope_node_tests {
    ...
    fn test_complete_multi_component_import() {
      ...
      assert_eq!(full_text, "com.example.service.MyService", "Should return complete import identifier");
    }
    ...
    fn test_complete_deep_nested_import() {
      ...
      let full_text = ts_file.get_text_from_node(&result.unwrap()).expect("Should get full text");
      assert_eq!(full_text, "com.company.project.module.submodule.feature.MyClass", "Should return complete deep import identifier");
    }
...
public class Test {}
      ...
      assert_eq!(result.unwrap().kind(), "import_declaration", "Found node should be import_declaration");
    }
...
public class Test {}"#;
      ...
      let result = add_import(&mut ts_file, &ImportInsertionPosition::AfterPackageDeclaration, "java.util", "List");
      assert!(result.is_some(), "Should successfully add import after package declaration");
      
      let new_content = ts_file.get_text_from_node(&ts_file.tree.as_ref().unwrap().root_node()).unwrap();
      assert!(new_content.contains("import java.util.List;"), "Should contain the new import");
      assert!(new_content.contains("package com.example;"), "Should preserve existing package declaration");
    }
...
public class Test {}"#;
      ...
      let result = add_import(&mut ts_file, &ImportInsertionPosition::BeforeFirstImport, "java.util", "List");
...
public class Test {}"#;
      ...
      let result = add_import(&mut ts_file, &ImportInsertionPosition::AfterLastImport, "java.util", "Map");
...
public class Test {}"#;
      ...
      let result = add_import(&mut ts_file, &ImportInsertionPosition::AfterLastImport, "java.util", "List");
    ...
    fn test_add_import_with_empty_parameters() {
      ...
      let result1 = add_import(&mut ts_file, &ImportInsertionPosition::AfterPackageDeclaration, "", "List");
      assert!(result1.is_none(), "Should not add import with empty package scope");
      
      let result2 = add_import(&mut ts_file, &ImportInsertionPosition::AfterPackageDeclaration, "java.util", "");
      assert!(result2.is_none(), "Should not add import with empty class name");
      
      let result3 = add_import(&mut ts_file, &ImportInsertionPosition::AfterPackageDeclaration, "   ", "   ");
...
public class Test {}"#;
      ...
      let result = add_import(&mut ts_file, &ImportInsertionPosition::BeforeFirstImport, "java.util", "List");
      assert!(result.is_some(), "Should successfully add import before first import without package");
...
public class Test {}"#;
      ...
      let result = add_import(&mut ts_file, &ImportInsertionPosition::AfterLastImport, "java.io", "File");
      assert!(result.is_some(), "Should successfully add import");
      
      let new_content = ts_file.get_text_from_node(&ts_file.tree.as_ref().unwrap().root_node()).unwrap();
      assert!(new_content.contains("import java.io.File;"), "Should contain properly formatted import");
      
      // Check that formatting is reasonable (though exact formatting may vary)
      let lines: Vec<&str> = new_content.lines().collect();
      let import_lines: Vec<&str> = lines.iter().filter(|line| line.contains("import")).cloned().collect();
...
public class MyClass {}"#;
      ...
      let add_result = add_import(&mut ts_file, &ImportInsertionPosition::AfterLastImport, "java.util", "Map");
      ...
      let duplicate_result = add_import(&mut ts_file, &ImportInsertionPosition::AfterLastImport, "java.util", "Map");
...
public class UserServiceImpl implements UserService {
    // Implementation details
      ...
      let add_result = add_import(&mut ts_file, &ImportInsertionPosition::AfterLastImport, "java.util.concurrent", "CompletableFuture");
      ...
      assert_eq!(updated_imports.len(), 6, "Should have six imports after addition to complex file");
    }
    ...
    fn test_edge_case_consistency() {
      ...
      for (code, description) in test_cases {
        ...
        let all_imports = get_all_import_declaration_nodes(&ts_file);
        assert!(all_imports.is_empty(), "Should return empty imports for edge case: {}", description);
        ...
        let _add_result = add_import(&mut ts_file, &ImportInsertionPosition::AfterPackageDeclaration, "java.util", "List");
